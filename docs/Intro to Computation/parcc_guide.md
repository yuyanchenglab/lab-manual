...
A simple repeatable guide to log in to Betty, PARCC's supercomputer, and get
oriented. Once you are comfortable here, see
[Connecting the Betty (PARCC) Cluster to Claude](betty_claude.html) for driving
the cluster with an agent.

Replace `<pennkey>` with your own PennKey throughout.

---

## Step 1 - Get a Kerberos ticket

Betty uses Kerberos for login. Grab a ticket, then follow the Duo prompt if you get one:
...
## Step 3 (optional, one time) - Set up an SSH key to skip repeated auth

Generating an SSH key and registering it means smoother future logins. On your local machine:
...
You can also add SSH multiplexing to `~/.ssh/config` so one connection is reused (sign in once, open many sessions). Add a `Host *.parcc.upenn.edu` block:
...
## Step 4 - You are on the login node, do not run heavy work here
...
* For real compute, request a node ([Step 7](#step-7---get-a-compute-node-interactive)).

---

## Step 5 - First orientation commands
...
## Step 6 - Know your two storage locations
...
Check live availability first with `parcc_sfree.py` or `sinfo`.
...
For training or long runs, do NOT use interactive. Write a script and `sbatch` it. This is the real Betty GPU template, based on the PARCC MNIST tutorial. Save it as `job.sbatch`:
